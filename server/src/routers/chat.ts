...
/**
 * 비동기 파일 저장 및 요약 처리
 * 답변을 보낸 후 백그라운드에서 실행되어 대화 속도에 영향을 주지 않습니다.
 */
async function runBackgroundTasks(history: any[], memory: any) {
  ...
  setImmediate(() => {
    try {
      ...
      console.log('[Background] 대화 기록 저장 완료');
    } catch (error) {
      console.error('[Background] 저장 에러:', error);
    }
  });

  // 2. 장기 기억 요약 (20개마다 비동기 실행)
  if (history.length - memory.messageCountAtLastSummary >= 20) {
    console.log('[Background] 장기 기억 요약 시작...');
    try {
      ...
      console.log('[Background] 장기 기억 요약 완료');
    } catch (error) {
  ...
  sendMessage: procedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (timeQuestionPattern.test(input.message)) {
        ...
        runBackgroundTasks(history, memory); // 백그라운드 작업 시작 (기다리지 않음)
      ...
      try {
        console.log('[Chat] DeepSeek-V4-Flash 호출 중...');
        const kstDateTime = getCurrentKSTString();
        const dynamicSystemPrompt = `${NUNA_SYSTEM_PROMPT}

[매우 중요 - 시간 안내 규칙]
- 현재 한국 시각: ${kstDateTime}
- 사용자가 시간을 물으면 위 시각을 정확히 말하세요.
...
${memory.summary || "아직은 우리 사이에 쌓인 추억이 많지 않네."}`;
        ...
        // 5. 어시스턴트 메시지 추가 및 백그라운드 작업 실행
        history.push({ role: 'assistant', content: responseText, createdAt: new Date() });
        
        // [핵심 최적화] 파일 저장과 요약 작업을 기다리지 않고 즉시 응답 반환
